...
int main(int argc,char *argv[]){
	...
	for(i = 0; i < argc; i++){ 
		...
		if(!strcmp(argv[i],"-s")){
			serverip = malloc((strlen(argv[++i])+1)*sizeof(char));//get buffer size of pipe
			strcpy(serverip,argv[i]);
		}
		if(!strcmp(argv[i],"-p"))
			serverport = atoi(argv[++i]);//get port
		if(!strcmp(argv[i],"-i")){
	...
	char command[buffer_size];
     char bufvar3[buffer_size];
	sprintf(bufvar3, "%s %d", serverip, serverport);
	printf("%s\n",bufvar3);
	//send server ip and port to workers
	 for(i=0; i< workers_num; i++){
		printf("parent write ok\n");
		sprintf(bufvar3, "%s %d %d", serverip, serverport,i);
		printf("%s\n",bufvar3);
		write(writer_fd[i],bufvar3,buffer_size-10);			
	}  
