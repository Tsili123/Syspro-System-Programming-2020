...
int main(int argc,char* argv[]){
    ...
    int server_port,portparam;
    char server_ip[50];
    //get server_ip and server_port
    sr = read(reader_fd,buffertemp,buffer_size-10);
    ...
    sscanf(buffertemp,"%s %d %d",server_ip,&server_port,&portparam); 
    //printf("%s\n",buffertemp);
    printf("ok worker%d server_ip: %s server_port: %d param: %d\n",getpid(),server_ip,server_port,portparam);
    ...
    /* strcpy(buffertemp,"good message\n");
    write(sock,buffertemp,buffer_size); */
	...
	// open each dir
		...
		while ((rdir_var = readdir(dir_var)) != NULL){ // open each file in dir
            ...
            sr = write(sock,bufferio,buffer_size-10);
...
//	}
   ...
   /*  while(1){
		...
		int sr= read(sock,msgbuf,buffer_size-10);
		if( sr < 0){
		...
		if(strncmp(msgbuf,"/listCountries",strlen("/listCountries"))==0){
			...
			sr= write(sock,dynam,buffer_size-10);//atomic read/write
		
			//sr = write(sock,buf,buffer_size+1);
			if( sr < 0){
				perror("error in read");
				errors++;
			}else{
			...
			//	arg[strcspn(arg, "\n")] = 0;
				...
				if ( sscanf(arg, "%[^ ] %[^ ] %[^ ] %s\n", virus,d1,d2,country)==3){
					...
					sr = write(sock,answer,90);//atomic read/write
		...
		if(sig_flag==1){
			...
			while ((rdir_var = readdir(dir_var)) != NULL){ // open each file in dir
	    		...
	    		fp = fopen(file,"r");//we open file with absolute path
				  ...
				  write(sock,file,300);//atomic read/write				
				}
			}
		}sig_flag2=0;
	}
